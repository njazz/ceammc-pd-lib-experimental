...
struct tl_toggle : public BaseGuiObject {
    t_canvas* canvas;
...
static void tl_toggle_getdrawparams(tl_toggle* x, t_object* /*view*/, t_edrawparams* params)
{
...
static void tl_toggle_action(TimelineData* x)
{
    outlet_float(reinterpret_cast<tl_toggle*>(x->object())->out1,1);
}

static void tl_toggle_default_action(TimelineData* x)
{
    //post ("def action");
    outlet_float(reinterpret_cast<tl_toggle*>(x->object())->out1,0);
}
...
//todo set arg - see ui.display
    ...
    tl_toggle* zx = (tl_toggle*)z;
    GuiFactory<tl_toggle>::ws_redraw(zx);
    zx->data->setXPos(zx->b_box.b_rect.x);
}

UI_fun(tl_toggle)::wx_paint(tl_toggle* zx, t_object* /*view*/)
{
    t_rect rect;
    zx->getRect(&rect);
    ...
    if (g) {
        ...
        etext_layout_set(zx->txt, "tl.toggle", zx->fnt, xoff, yoff, rect.width, rect.height / 2,
...
UI_fun(tl_toggle)::wx_oksize(tl_toggle* z, t_rect* newrect)
{
...
UI_fun(tl_toggle)::new_ext(tl_toggle* zx, t_symbol* /*s*/, int /*argc*/, t_atom* /*argv*/)
{
    zx->canvas = canvas_getcurrent();

    zx->data = new TimelineData(zx->canvas, (t_object*)zx);
    zx->data->setXPos(zx->b_box.b_obj.o_obj.te_xpix);
    zx->data->setAction(&tl_toggle_action);
    zx->data->setDefaultAction(&tl_toggle_default_action);
...
UI_fun(tl_toggle)::init_ext(t_eclass* z)
{
    z->c_widget.w_displacefn = tl_cue_displace;
    eclass_addmethod(z, reinterpret_cast<method>(tl_toggle_getdrawparams), "getdrawparams", A_NULL, 0);
...
UI_fun(tl_toggle)::free_ext(tl_toggle* zx)
{
    // CICM cleanup
...
extern "C" void setup_tl0x2etoggle()
{
    GuiFactory<tl_toggle> class1;
    class1.setup("tl.toggle");
}
