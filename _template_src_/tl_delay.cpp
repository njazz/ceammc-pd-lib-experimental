...
struct tl_delay : public BaseGuiObject {
    t_canvas* canvas;
...
static void tl_delay_getdrawparams(tl_delay* x, t_object* /*view*/, t_edrawparams* params)
{
...
static void tl_delay_action(TimelineData* x)
{
    //outlet_delay(reinterpret_cast<tl_delay*>(x->object())->out1);
    clock_delay(x->clock_, x->delayTime_);
}


static void tl_delay_default_action(TimelineData* x)
{
    //outlet_delay(reinterpret_cast<tl_delay*>(x->object())->out1);
    clock_unset(x->clock_);
}

//

static void tl_delay_tick(tl_delay *x)
{
    outlet_bang(x->out1);
}

static void tl_delay_bang(tl_delay *x)
{
    clock_delay(x->x_clock, x->delayTime_);
}

static void tl_delay_stop(tl_delay *x)
{
...
//todo set arg - see ui.display
    ...
    tl_delay* zx = (tl_delay*)z;
    GuiFactory<tl_delay>::ws_redraw(zx);
    zx->data->setXPos(zx->b_box.b_rect.x);
}

UI_fun(tl_delay)::wx_paint(tl_delay* zx, t_object* /*view*/)
{
    t_rect rect;
    zx->getRect(&rect);
    ...
    if (g) {
        ...
        etext_layout_set(zx->txt, "tl.delay", zx->fnt, xoff, yoff, rect.width, rect.height / 2,
...
UI_fun(tl_delay)::wx_oksize(tl_delay* z, t_rect* newrect)
{
...
UI_fun(tl_delay)::init_ext(t_eclass* z)
{
    z->c_widget.w_displacefn = tl_cue_displace;
    eclass_addmethod(z, reinterpret_cast<method>(tl_delay_getdrawparams), "getdrawparams", A_NULL, 0);
...
UI_fun(tl_delay)::free_ext(tl_delay* zx)
{
    // CICM cleanup
...
extern "C" void setup_tl0x2edelay()
{
    GuiFactory<tl_delay> class1;
    class1.setup("tl.delay");
}
