...
namespace tl {
    ...
    void TimelineData::setDefaultAction(UIAction fn)
    {
        cbd_ = fn;
    }

    void TimelineData::triggerDefaultAction()
    {
        if (!cbd_)
            return;
    ...
    ParameterNameList UIStorage::parameterNames_;

    bool UIStorage::parameterAdd(std::string name)
    {
        if (name == "") {
            LIB_ERR << "Bad parameter name";
            return false;
        }

        // no error if exists
        if (UIStorage::parameterExists(name))
            return true;
    ...
    std::string UIStorage::parameterAt(size_t pos)
    {
        return (pos < parameterNames_.size()) ? parameterNames_[pos] : "";
    }

    bool UIStorage::parameterExists(std::string name)
    {
        ParameterNameList::iterator it = std::find(parameterNames_.begin(), parameterNames_.end(), name);
        return it != parameterNames_.end();
    }
    void UIStorage::parameterRemove(std::string name)
    {
        ParameterNameList::iterator it = std::find(parameterNames_.begin(), parameterNames_.end(), name);
        if (it != parameterNames_.end())
            parameterNames_.erase(it);
    }

    size_t UIStorage::parameterSize()
    {
        return parameterNames_.size();
    }

    size_t UIStorage::storedValueCount()
    {
        size_t ret = 0;

        for (UIDataList::iterator it = data_.begin(); it != data_.end(); ++it)
        {
            ret += (*it)->hasValue();
        }
