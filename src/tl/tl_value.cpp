...
struct tl_value : public BaseGuiObject {
    t_canvas* canvas;
...
static void tl_value_getdrawparams(tl_value* x, t_object* /*view*/, t_edrawparams* params)
{
...
static void tl_value_action(TimelineData* x)
{
//    outlet_bang(reinterpret_cast<tl_value*>(x->object())->out1);
}
...
//todo set arg - see ui.display
    ...
    tl_value* zx = (tl_value*)z;
    GuiFactory<tl_value>::ws_redraw(zx);
    zx->data->setXPos(zx->b_box.b_rect.x);
}

UI_fun(tl_value)::wx_paint(tl_value* zx, t_object* /*view*/)
{
    t_rect rect;
    zx->getRect(&rect);
    ...
    if (g) {
        ...
        etext_layout_set(zx->txt, "tl.value", zx->fnt, xoff, yoff, rect.width, rect.height / 2,
...
UI_fun(tl_value)::wx_oksize(tl_value* z, t_rect* newrect)
{
...
UI_fun(tl_value)::new_ext(tl_value* zx, t_symbol* s, int argc, t_atom* argv)
{
    zx->canvas = canvas_getcurrent();

    zx->data = new TimelineData(zx->canvas, (t_object*)zx);
    zx->data->setXPos(zx->b_box.b_obj.o_obj.te_xpix);
    zx->data->setAction(&tl_value_action);

    zx->data->setValueSelector(s);
    zx->data->setValueData(new ceammc::AtomList(argc,argv));
...
UI_fun(tl_value)::init_ext(t_eclass* z)
{
    z->c_widget.w_displacefn = tl_cue_displace;
    eclass_addmethod(z, reinterpret_cast<method>(tl_value_getdrawparams), "getdrawparams", A_NULL, 0);
...
UI_fun(tl_value)::free_ext(tl_value* zx)
{
    // CICM cleanup
...
extern "C" void setup_tl0x2evalue()
{
    GuiFactory<tl_value> class1;
    class1.setup_noin("tl.value");
}
